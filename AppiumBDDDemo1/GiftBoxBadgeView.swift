//
//  GiftBoxBadgeView.swift
//  HSBCRewards
//
//  Created by hans on 2018/2/7.
...
private var badgeLabelKey = "badgeLabelKey"

public struct Badge<Base> {
...
public extension UIView {
    public var badge: Badge<UIView> {
        return Badge(self)
    }
}

// MARK: - add Badge
public extension Badge where Base: UIView {
    ...
    public func moveBadge(x: CGFloat, y: CGFloat) {
         ...
         */
        self.base.badgeLabel.bX = (self.base.bWidth - self.base.badgeLabel.bHeight*0.5)/*badge的x坐标*/ + x;
        self.base.badgeLabel.bY = -self.base.badgeLabel.bHeight*0.5/*badge的y坐标*/ + y;
    }
    ...
    public func setBadgeHeight(points: CGFloat) {
        let scale = points/self.base.badgeLabel.bHeight
    ...
    public func setBadgeLabel(attributes: (HSBCBadgeLabel)->()) {
...
extension UIView {
    ...
    var badgeLabel: HSBCBadgeLabel {
        get {
            if let aValue = objc_getAssociatedObject(self, &badgeLabelKey) as? HSBCBadgeLabel {
                return aValue
            } else {
                let defaultBadgeLabel = HSBCBadgeLabel.defaultBadgeLabel()
                defaultBadgeLabel.center = CGPoint(x: self.bWidth - 8, y: 0)
        ...
        set {
            objc_setAssociatedObject(self, &badgeLabelKey, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    
    func updateBadgeLayout() {
    }
